...
---
name: doc-index-local
description: 本地固定路径文档索引工具。为指定目录（如 D:\我的文档）生成文件索引，包含 index.html（搜索界面）、scan_files.py（扫描脚本）、files_index.json（索引数据）。支持实时搜索、类型/目录筛选、分页、列表/网格视图。
---

# 本地文档索引工具（固定路径版）

为本地固定目录生成文件索引，提供 web 界面搜索。

## 功能特点

- 扫描指定目录，生成文件索引
- 自包含 HTML 界面（内嵌数据，无需服务器）
- 实时搜索、类型/目录筛选
- 列表/网格双视图
- 分页显示（每页 60 条）
- 点击直接打开文件（file:// 协议）
...
"""
文档索引扫描脚本 + 生成自包含 HTML
解决 file:// 协议无法 fetch 本地 JSON 的问题
"""
...
ROOT_DIR = r"D:\我的文档"  # ← 修改这里为你要扫描的目录
...
SKIP_DIRS = {
    "360js Files", "KingsoftData", "MuMu共享文件夹", "Tencent Files",
    "WPS Cloud Files", "WPSDrive", "WeChat Files", "WindowsPowerShell",
    "pptvclt", "videorecord_win", "xwechat_files", "AppWebCache",
    "__pycache__", ".git", "node_modules", "文档索引"
}
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .tip-bar {
    background: rgba(79,127,255,.08); border: 1px solid rgba(79,127,255,.2);
...
<header>
  <div class="logo">
    ...
    <div>
      <div class="logo-text">我的文档</div>
      <div class="logo-sub">文件索引中心</div>
    </div>
  </div>
  <div class="search-wrap">
    ...
    <input type="text" id="searchInput" placeholder="搜索文件名、目录… (Ctrl+F)" autocomplete="off" spellcheck="false">
...
<main>
  ...
  <div class="content">
    <div class="tip-bar">
      💡 双击运行 <strong>update_index.bat</strong> 可重新扫描更新索引；点击文件名直接打开文件
    </div>
...
function init() {
  document.getElementById("scanTime").textContent = "索引时间: " + INDEX_DATA.scan_time;
  buildSidebar();
  renderStats();
  applyFilter();
...
function openFile(path) {
  const uri = "file:///" + path.replace(/\\/g,"/").replace(/ /g,"%20");
...
- 首次运行会扫描所有文件，耗时较长（取决于文件数量）
- 生成的 `index.html` 是自包含的，可以复制到其他地方使用
- 如果文件较多（>10000），建议定期重新扫描更新索引
