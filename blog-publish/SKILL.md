...
每次发布从以下主题池中按顺序轮换（参见 `references/theme-rotation.md`）：
...
按 `references/article-template.md` 中的模板和结构生成文章，要求：

1. **技术含量高**：深入讲解技术细节，提供实用价值
2. **配图丰富**：使用 Markdown 或 HTML 嵌入相关技术图表、代码示例
3. **无 AI 味**：自然流畅的中文表达，避免机械化的 AI 特征（不用"首先""其次"开头的段落，不用"总的来说"等套话）
...
curl -X POST "http://43.226.44.9/wp-json/wp/v2/posts" \
  ...
  -d '{
    "title": "文章标题",
    "content": "文章内容（HTML格式，支持<p><code><blockquote>等标签）",
    "status": "publish",
    "excerpt": "文章摘要，100字以内"
  }'
```
...
- 内容必须为 HTML 格式，而非纯 Markdown
- 响应中的 `id` 字段即为文章 ID，记录用于日志

### 第四步：验证结果

检查 curl 响应的 HTTP 状态码：
- `201 Created`：发布成功
- `401 Unauthorized`：认证失败，检查用户名和应用密码
- `403 Forbidden`：权限不足，确认用户有发布权限
...
详见 `references/article-template.md`。
...
使用 `automation_update` 工具（`scheduleType: "recurring"`）：
- 名称：`每日自动发布技术文章`
- 时间：`FREQ=DAILY;BYHOUR=9;BYMINUTE=0`（每天上午 9 点）
- 每日发布 2 篇（创建两条自动化规则，或一条规则内循环两次）
...
此 Skill 设计为**无需用户确认**：
- 生成内容后直接发布，不输出草稿给用户审阅
- 不使用 `draft` 状态，全部 `publish`
- 错误时自动重试一次，仍失败则记录错误日志但不阻塞后续任务
...
| 问题 | 原因 | 解决 |
|------|------|------|
| `401 Unauthorized` | 应用密码错误或格式有误 | 确认密码无空格，重新在 WordPress 后台生成 |
